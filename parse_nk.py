...
import disfluency.chart_helper as chart_helper
import disfluency.nkutil as nkutil

import disfluency.trees as trees
...
class MultiLevelEmbedding(nn.Module):
    ...
    def forward(self, xs, batch_idxs, extra_content_annotations=None):
        ...
        if self.partitioned:
            if content_annotations.shape[0] != timing_signal.shape[0]:
                # print(content_annotations.shape)
                # print(timing_signal.shape)
                # print()
                _max_len = max(content_annotations.shape[0], timing_signal.shape[0])
                if content_annotations.shape[0] != _max_len:
                    _pad = _max_len - content_annotations.shape[0]
                    content_annotations = F.pad(content_annotations.clone(), (0, 0, math.floor(_pad/2), math.ceil(_pad/2)), "constant", 0)
                else:
                    _pad = _max_len - timing_signal.shape[0]
                    timing_signal = F.pad(timing_signal.clone(), (0, 0, math.floor(_pad/2), math.ceil(_pad/2)), "constant", 0)
...
class NKChartParser(nn.Module):
    ...
    def __init__(
        ...
        if hparams.use_chars_lstm:
            ...
            self.bert_tokenizer, self.bert = get_bert(os.path.abspath(os.path.join(ABS_ROOT, "disfluency", hparams.bert_model)), hparams.bert_do_lower_case)
    ...
    def parse_batch(self, sentences, golds=None, return_label_scores_charts=False, get_annotations=False):
        ...
        if self.char_encoder is not None:
            ...
            for snum, sentence in enumerate(sentences):
                ...
                word_token_index.append([_token_index])
                ...
                for word in cleaned_words:
                    ...
                    for _ in range(len(word_tokens)):
                        ...
                        _w_i.append(_token_index)
                    word_token_index.append(_w_i)
                ...
                # _tokens = self.bert_tokenizer.convert_ids_to_tokens(input_ids)
            ...
            del _
            features = all_encoder_layers[-1] # deeper layer has semantic information, we take the output of the last layer
        ...
        if self.encoder is not None:
            ...
            # Save annotations
            if get_annotations:
                # Save annotations and bert last layer hidden states
                return (annotations, features, word_token_index) 
